...
class _AdicionesScreenState extends State<AdicionesScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _cargar() async {
    ...
    try {
      final data = await ApiService.get('/api/adiciones');
  ...
  List<Map<String, dynamic>> get _filtrados {
    ...
    return _items
        .where((a) => (a['nombre']?.toString().toLowerCase().contains(q) ?? false))
        .toList();
  }

  Future<void> _toggleEstado(Map<String, dynamic> item) async {
    final id = item['id_adicion'] ?? item['id'];
    final nuevoEstado = (item['estado'] == true || item['estado'] == 1) ? 0 : 1;
    try {
      await ApiService.patch('/api/adiciones/$id/estado', {'estado': nuevoEstado});
      setState(() {
        final idx = _items.indexWhere((a) => (a['id_adicion'] ?? a['id']) == id);
  ...
  Future<void> _eliminar(Map<String, dynamic> item) async {
    ...
    try {
      await ApiService.delete('/api/adiciones/$id');
      setState(() => _items.removeWhere((a) => (a['id_adicion'] ?? a['id']) == id));
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Adición eliminada'),
  ...
  void _abrirDetalle(Map<String, dynamic> item) {
    showDialog(
      context: context,
      builder: (_) => _AdicionDetalleDialog(item: item, fmt: _fmt),
    );
  }
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${_items.length} adiciones registradas',
                        style: GoogleFonts.nunito(
          ...
          Expanded(
            child: _loading
                ...
                : _error != null
                    ...
                    : filtrados.isEmpty
                        ? Center(
                            child: Text('No se encontraron adiciones',
                                style: GoogleFonts.nunito(
                                ...
                                child: RefreshIndicator(
                                  ...
                                  child: ListView.separated(
                                    ...
                                    itemBuilder: (context, i) {
                                      ...
                                      final precioDouble = precio is num
                                          ? precio.toDouble()
                                          : double.tryParse(
                                                  precio?.toString() ?? '0') ??
                                              0;
...
class _AdicionFormScreenState extends State<_AdicionFormScreen> {
  ...
  void initState() {
    ...
    final precio = widget.item?['precio'];
    _precioCtrl  = TextEditingController(
        text: precio != null ? precio.toString() : '');
                  ...
                  children: [
                    ...
                    _FormField(
                      ...
                      child: TextField(
                        ...
                        decoration: _inputDec('Nombre de la adición',
                    ...
                    _FormField(
                      ...
                      child: TextField(
                        ...
                        style: GoogleFonts.nunito(fontSize: 14),
                        decoration: _inputDec('Ej: 300g, 200ml...'),
                      ),
                    ),
                    const SizedBox(height: 14),
                    ...
                    _FormField(
                      ...
                      child: TextField(
                        ...
                        onChanged: (_) =>
                            setState(() => _errores.remove('precio')),
                        style: GoogleFonts.nunito(fontSize: 14),
                        decoration:
                            _inputDec('0', error: _errores['precio']).copyWith(
                          prefixText: '\$ ',
                    ...
                    if (_errores['precio'] != null)
                      _errMsg(_errores['precio']!),
                    const SizedBox(height: 14),
...
class _AdicionDetalleDialog extends StatelessWidget {
  final Map<String, dynamic> item;
  final NumberFormat fmt;
  const _AdicionDetalleDialog({required this.item, required this.fmt});
          ...
          children: [
            ...
            RichText(
              ...
              text: TextSpan(
                ...
                children: [
                  const TextSpan(text: '¿Eliminar la adición '),
                  TextSpan(
                      text: '"$nombre"',
