...
class _AdminPedidosScreenState extends State<AdminPedidosScreen> {
  ...
  // Sin 'entregado' -- ese estado ya generó plata real y vive en la
  // pantalla Ventas, así que no tiene sentido ofrecerlo en este filtro.
  ...
  List<Pedido> get _ventasFiltradas {
    // Pedidos muestra todo excepto 'entregado' -- ese estado vive en Ventas.
  ...
  Widget build(BuildContext context) {
    ...
    final pedidosCount = _ventas.where((v) => v.estado != 'entregado').length;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '$pedidosCount registro${pedidosCount != 1 ? 's' : ''} en total',
...
class _VentaRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 'entregado' nunca aparece en Pedidos (vive en Ventas), así que esa
    // rama de las condiciones originales queda fuera aquí.
    final mostrarDevolver = venta.estado == 'despachado';
  ...
  void initState() {
    ...
    _items = p.lineas.map((l) {
      ...
      return _ItemEdit(
        ...
        // feature opcional de untables que permiteSalsas protege -- por
        // eso los Bowl siempre tienen permiteSalsas=false en BD y el
        // gate normal los dejaría sin cobertura.
        salsas: (l.esBowl || permiteSal) ? l.salsas : [],
        ...
        // arriba) -- texto libre, se precarga siempre tal cual.
  ...
  Future<void> _crear() async {
    ...
    try {
      ...
      final items = _carritoItems.map((item) => {
        ...
        if (item.frutas != null) 'frutas':       item.frutas,
        if (item.observacion != null) 'observacion': item.observacion,
      }).toList();
                ...
                itemBuilder: (gctx, idx) {
                  ...
                  return GestureDetector(
                    onTap: () async {
                      ...
                      final frutas = result.tipoFrutas;
                      final observacion = result.observacion;
                      ...
                      final sameIdx = _carritoItems.indexWhere((i) {
                        ...
                        if ((i.frutas ?? '') != (frutas ?? '')) return false;
                        if ((i.observacion ?? '') != (observacion ?? '')) return false;
    ...
    return Column(
      ...
      children: [
              ...
              ..._carritoItems.map((item) => Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            '${item.cantidad}x ${item.nombreCompleto}',
                            style: const TextStyle(fontSize: 13),
                          ),
                        ),
                        Text(
        ...
        Builder(builder: (context) {
          ...
          final transferenciaChip = GestureDetector(
            onTap: () => setState(() {
              _metodoPago = 'transferencia';
              _montoEfectivo = 0;
              _montoTransfer = 0;
          ...
          final datafonoChip = _PagoChip(
              label: 'Datafono',
              icon: Icons.credit_card,
              selected: _metodoPago == 'datafono',
              onTap: () => setState(() {
                    _metodoPago = 'datafono';
                    _montoEfectivo = 0;
                    _montoTransfer = 0;
          ...
          if (_datafonoHabilitado) {
            return Column(children: [
              Row(children: [
                ...
                Expanded(child: transferenciaChip),
              ]),
              const SizedBox(height: AppSizes.sm),
              Row(children: [
                mixtoChip,
